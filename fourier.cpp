...
// files to read and write, .bin or .txt:
...
// number of elements in the big signal:
#define NUMELEMENTS	(1*1024*1024)

// max number of periods considered
#define MAXPERIODS	100

// file type
#define BINARY

// print debugging messages option
#define DEBUG		true
...
main( int argc, char *argv[ ] )
{
    ...
    {
        BigSignal = new float [NUMELEMENTS];		// duplicate part of the array
    ...
    if( me == BOSS )
    {
        BigSums = new float [MAXPERIODS];	// stores all the sums
    }

    // start the timer:
    double time0 = MPI_Wtime( );

    // have the BOSS send to itself:
    if( me == BOSS )
    {
    ...
    else
    {
        ...
        MPI_Send( BigSums, MAXPERIODS, MPI_FLOAT, BOSS, TAG_GATHER, MPI_COMM_WORLD );
    }
    ...
    if( me == BOSS )
    {
        ...
        for( int src = 0; src < NumCpus; src++ )
        {
            ...
            MPI_Recv( tmpSums, MAXPERIODS, MPI_FLOAT, dst, TAG_GATHER, MPI_COMM_WORLD, &status );
            for( int s = 0; s < MAXPERIODS; s++ )
                BigSums[s] += tmpSums[s];
        }
    }
    ...
    if( me == BOSS )
    {
        ...
        // compute mega-mults computed per second
